...
class SaleOrder(models.Model):
    ...
    def _get_workflow_copy_mode(self):
        return (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("sale_automatic_workflow.workflow_copy_mode", "no_copy")
        )

    def copy_data(self, default=None):
        vals_list = super().copy_data(default=default)
        # ``workflow_process_id`` has ``copy=False`` by default. When the global
        # ``workflow_copy_mode`` setting is "copy", propagate the origin workflow on
        # duplication (this also covers sale order splits, which go through ``copy``).
        # ``setdefault`` keeps any value explicitly forced by the caller.
        mode = self._get_workflow_copy_mode()
        if mode == "copy":
