...
class SaleOrderLine(models.Model):
    ...
    def _get_last_sale(self):
        ...
        # TODO ask is there other way to search
        # with date filter in datetime field
        ...
        last_date_lines = self.env['sale.order.line'].search(
            [('product_id', '=', self.product_id.id),
             ...
             ('state', 'in', ['sale', 'done'])])
        # at least one line found before checked at row 73
    ...
    last_sale_qty = fields.Integer(
        string='Last Sale Quantity', compute='_get_last_sale')
